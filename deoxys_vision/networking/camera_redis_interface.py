...
class CameraRedisPubInterface:
    ...
    def __init__(self, redis_host="172.16.0.1", redis_port=6379, camera_id=0, custom_camera_name=None):
        ...
        self.info_redis = redis.StrictRedis(
            host=self.redis_host, port=self.redis_port, charset="utf-8", decode_responses=True
        )

        self.img_redis = redis.StrictRedis(
            host=self.redis_host, port=self.redis_port, charset="utf-8", decode_responses=False
        )
